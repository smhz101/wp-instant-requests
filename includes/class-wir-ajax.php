...
class WIR_Ajax {
    public static function submit() {
        ...
        if (is_wp_error($post_id)) wp_send_json_error($post_id->get_error_message(), 500);
        ...
        update_post_meta($post_id, '_wir_thread', [[
            'type'    => 'user',
        ...
        if (!empty($o['recaptcha_secret'])) {
            ...
            if (!$ok) wp_send_json_error(__('Captcha failed.','wp-instant-requests'),400);
        }
        ...
        if ($o['notify'] === 'yes') {
            ...
            $body = sprintf(
                "Name: %s\nEmail: %s\nTopic: %s\nObject ID: %d\n\nMessage:\n%s\n\nView: %s",
                $name, $email, $topic ?: '-', $pid, wp_strip_all_tags($msg),
                admin_url('post.php?post=' . $post_id . '&action=edit')
            );
            wp_mail($to, $subject, $body);
        }

        do_action('wir_request_created', $post_id, [
            'name'=>$name,'email'=>$email,'topic'=>$topic,'product_id'=>$pid,'message'=>wp_strip_all_tags($msg)
        ]);
