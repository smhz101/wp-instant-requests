...
class WIR_Replies {
    public static function metabox() {
        add_meta_box('wir_details', __('Request Details & Reply', 'wp-instant-requests'), [__CLASS__, 'render'], WIR_Plugin::CPT, 'normal', 'high');
    }

    public static function render($post) {
        if (!current_user_can('edit_wir_request', $post->ID)) {
            echo esc_html__('You do not have permission.', 'wp-instant-requests'); return;
        }
    ...
    public static function save($post_id) {
        if (!isset($_POST[WIR_Plugin::NONCE]) || !wp_verify_nonce($_POST[WIR_Plugin::NONCE], WIR_Plugin::NONCE)) return;
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
        if (!current_user_can('edit_wir_request', $post_id)) return;
        ...
        if ($email && is_email($email)) wp_mail($email, $subject, $body);
