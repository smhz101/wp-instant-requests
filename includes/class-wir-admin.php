...
class WIR_Admin {
    ...
    public static function ajax_get_thread(){
        ...
        if (!$items) {
            $content = wp_strip_all_tags(get_post_field('post_content', $id));
            if ($content !== '') {
                $items = [['type'=>'user','message'=>$content,'time'=> get_post_time('U', true, $id)]];
    ...
    public static function ajax_save_note(){
        ...
        $id=absint($_POST['request_id']??0); $note=wp_kses_post(wp_unslash($_POST['note']??''));

        if(!$id || $note==='') wp_send_json_error('Invalid',400);

        $items=self::load_thread($id);

        $items[]=['type'=>'note','message'=>$note,'time'=>time()];

        self::save_thread($id,$items);

        // Also store last notes text (optional separate meta)
        $notes = get_post_meta($id,'_wir_notes',true); if(!is_array($notes)) $notes=[];

        $notes[]=['user'=>get_current_user_id(),'note'=>wp_strip_all_tags($note),'time'=>time()];
    ...
    public static function ajax_toggle_status(){
        if (!current_user_can('edit_wir_requests')) wp_send_json_error('denied',403);
        check_ajax_referer('wir_admin_nonce','nonce');
        $id=absint($_POST['request_id']??0);
        ...
        wp_send_json_success(['status'=>$next]);
    }

    public static function ajax_assign_me(){
        if (!current_user_can('edit_wir_requests')) wp_send_json_error('denied',403);
        check_ajax_referer('wir_admin_nonce','nonce');
        $id=absint($_POST['request_id']??0); $uid=get_current_user_id();
        update_post_meta($id,'_wir_assigned',$uid);
        $u = get_user_by('id',$uid);
        wp_send_json_success(['name'=>$u ? $u->display_name : '']);
    }
    ...
    public static function menus() {
        add_menu_page(
            ...
            'edit_wir_requests',
            'wir',
            [__CLASS__, 'page_requests'], // Custom mailbox page at top-level
        ...
        // AJAX: admin reply
        add_action('wp_ajax_wir_admin_reply', [__CLASS__, 'ajax_admin_reply']);
    }

    /** Load CSS/JS only on our pages */
    public static function enqueue_admin_assets() {
        $page = isset($_GET['page']) ? sanitize_key($_GET['page']) : '';
        if (!in_array($page, ['wir', 'wir-settings'], true)) return;

        wp_enqueue_style('wir-admin', WIR_URL . 'assets/css/admin.css', [], WIR_VERSION);
        wp_enqueue_script('wir-admin', WIR_URL . 'assets/js/admin.js', ['jquery'], WIR_VERSION, true);

        wp_localize_script('wir-admin', 'WIRAdmin', [
                ...
                <div class="wir-filters">
                    <select name="topic">
                      ...
                      <select name="status">
                        ...
                        <?php foreach (['open','replied','closed'] as $_s): ?>
                    ...
                    <a class="button" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array_merge($_GET,['action'=>'wir_export_csv']), admin_url('admin-post.php') ), 'wir_export_csv' ) ); ?>">
                    ...
                    <?php while ($q->have_posts()): $q->the_post();
                        ...
                        <!-- <div class="wir-item"
                            ...
                            <div class="wir-item-sub">
                                <?php if ($topic): ?><span class="wir-badge"><?php echo esc_html($topic); ?></span><?php endif; ?>
                                <?php if ($title): ?><span class="wir-dim">· <?php echo esc_html($title); ?></span><?php endif; ?>
                            </div>
    ...
    public static function register_settings() {
        register_setting('wir_settings', 'wir_settings', function ($input) {
            $out = [];
            ...
            $out['side'] = in_array($side, ['left','right'], true) ? $side : 'right';
            ...
            $out['show_on'] = in_array($show, ['product','single','both'], true) ? $show : 'product';
    ...
    public static function settings_page() {
        if (!current_user_can('manage_options')) wp_die(__('You do not have permission.', 'wp-instant-requests'));
    ...
    public static function ajax_admin_reply() {
        ...
        if (!$request_id || '' === trim($message)) wp_send_json_error(__('Invalid data.', 'wp-instant-requests'), 400);
        if (get_post_type($request_id) !== WIR_Plugin::CPT) wp_send_json_error(__('Invalid request ID.', 'wp-instant-requests'), 400);
        ...
        $sent = false;
        if ($email && is_email($email)) $sent = wp_mail($email, $subject, $body);
    ...
    public static function export_csv(){
        if (!current_user_can('edit_wir_requests')) wp_die('denied');
        check_admin_referer('wir_export_csv');

        // Rebuild same query from $_GET
        $args = [
        'post_type'=>'wir_request','posts_per_page'=>-1,'orderby'=>'date','order'=>'DESC',
        's'=> sanitize_text_field($_GET['s'] ?? '')
        ];
        $meta = [];
        if (!empty($_GET['topic']))  $meta[]=['key'=>'_wir_topic','value'=>sanitize_text_field($_GET['topic']),'compare'=>'='];
        if (!empty($_GET['status'])) $meta[]=['key'=>'_wir_status','value'=>sanitize_text_field($_GET['status']),'compare'=>'='];
        if (!empty($_GET['mine']))   $meta[]=['key'=>'_wir_assigned','value'=>get_current_user_id(),'compare'=>'='];
        ...
        fputcsv($out, ['ID','Date','Name','Email','Topic','Product','Status','Assignee','Message']);
        ...
        fputcsv($out, [
            ...
            (function($uid){ $u=get_user_by('id',(int)$uid); return $u?$u->display_name:''; })(get_post_meta($id,'_wir_assigned',true)),
